...
OpenAMは、リソースへのアクセスを許可するか、拒否するかといった、認可決定に到達するためのポリシーに依存しています。OpenAMのポリシーエージェントはポリシー強制ポイント（PEP）として動作するのに対し、OpenAMは、ポリシー決定ポイント（PDP）として機能します。つまり、ポリシーエージェントや他のPEPは、OpenAMによってレンダリングされるポリシー決定を実施するためだけの責任を取ります。OpenAMにアプリケーションとそのポリシーを設定した場合、ポリシー管理ポイント（PAP）としてOpenAMを使用します。
